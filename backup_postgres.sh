...
calcular_tamanho_dump() {
    ...
    tamanho_total=$(( $tamanho_dump+$tamanho_margem ))
...
for db in $base_dados; do
    ...
    if  [ "$resultado" -eq 1 ]; then
        ...
        # Realiza o dump do PostgreSQL e compacta o arquivo SQL em gzip diretamente
        # Redireciona a saída de erro para o arquivo de log
        pg_dump -h "$db_servidor" -U "$db_user" -v -C -w "$db" 2> "$log_file" | gzip > "$backup_file.sql.gz"
        wait
